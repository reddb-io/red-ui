...
const PRESETS: Connection[] = [
  { id: 'embedded', label: 'Embedded', url: 'http://localhost:5055', role: 'embedded' },
  { id: 'docker-primary', label: 'Docker primary', url: 'http://localhost:15055', role: 'primary' },
]
...
describe('LocalUrlProvider — implementation-specific', () => {
  it('list() merges presets and history (presets first, dedup by url)', async () => {
    ...
    history.save([{ url: 'http://10.0.0.5:5055', label: '10.0.0.5', last_used: 1 }])
    const p = new LocalUrlProvider({ presets: PRESETS, history, clientFactory: fakeClient })
  ...
  it('connect raises UnreachableConnectionError when ping fails', async () => {
    ...
    await expect(p.connect('embedded')).rejects.toBeInstanceOf(UnreachableConnectionError)
  })
  ...
  it('history is capped at historyMax', async () => {
    ...
    expect(history.load().map((e) => e.url)).toEqual(['http://c:5055', 'http://b:5055'])
  })

  it('forget removes a URL from history', async () => {
    const history = memoryHistory()
    const p = new LocalUrlProvider({ presets: [], history, clientFactory: fakeClient })
    await p.connect('http://a:5055')
