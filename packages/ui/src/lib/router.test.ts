...
describe('targetToHref', () => {
  ...
  it('maps a collection target to its collection-page href', () => {
    expect(targetToHref({ view: 'collection', collection: 'users', subpage: 'table' })).toBe(
      '/c/users/p/table',
    )
  })
})
...
describe('targetToLocation', () => {
  it('flattens a collection target into the collections view', () => {
    expect(targetToLocation({ view: 'collection', collection: 'users', subpage: 'graph' })).toEqual(
      { view: 'collections', collection: 'users', subpage: 'graph' },
    )
  })
...
describe('pathToLocation', () => {
  ...
  it('falls back to the collections workspace for / and unknown paths', () => {
    expect(pathToLocation('/')).toEqual({ view: 'collections', collection: null, subpage: null })
...
describe('createStateRouter', () => {
  ...
  it('honours an initial location', () => {
    const r = createStateRouter({ view: 'cluster', collection: null, subpage: null })
