...
describe("parseGraphExport — model", () => {
  ...
  it("prefers the versioned contract embedded at graph.json#contract", () => {
    const loaded = parseGraphExport({
      nodes: [{ id: "diagnostic-only" }],
      edges: [],
      contract: {
        version: "1.0.0",
        nodes: [
        ...
        stats: {
          ...
          node_types: { file: 1, symbol: 1 },
        },
      },
    });

    expect(loaded.version).toBe("1.0.0");
    expect(loaded.model.nodes.map((n) => n.id)).toEqual(["1", "2"]);
    expect(loaded.model.nodes[0].data).toMatchObject({
      description: "auth module",
      exports: ["issueToken"],
      orphan: true,
    });
...
describe("parseGraphExport — malformed / wrong version", () => {
  ...
  it("rejects an unsupported major version with a clear message", () => {
    expect(() =>
      ...
      /Unsupported graph export version "2.0"[\s\S]*supports version 1\.x/
    );
  });
...
describe("parseGraphExportJson", () => {
  ...
  it("wraps JSON syntax errors as GraphContractError", () => {
    expect(() => parseGraphExportJson("{ not json ")).toThrow(
      GraphContractError
    );
...
describe("loadGraphExport", () => {
  it("fetches and parses a graph from a URL", async () => {
    const fetchFn = (async () =>
      new Response(JSON.stringify(validExport()), {
        status: 200,
      })) as typeof fetch;
    const loaded = await loadGraphExport("https://example.test/graph.json", {
      fetch: fetchFn,
    });
    expect(loaded.model.nodes).toHaveLength(3);
    expect(hasGraphShape(loaded.result)).toBe(true);
  ...
  it("surfaces contract errors from fetched-but-wrong-version payloads", async () => {
    const fetchFn = (async () =>
      new Response(
        JSON.stringify({ version: "9.9", graph: { nodes: [], edges: [] } }),
        {
          status: 200,
        }
      )) as typeof fetch;
